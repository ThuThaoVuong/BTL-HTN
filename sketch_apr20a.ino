...
const uint32_t TRACK_HOLD_MS   = 15000;
const uint32_t TRACK_REMOVE_MS = 20000;
...
<script>
 let socket;
  ...
  socket.onclose = () => {
    console.log("⚠️ WebSocket CLOSED → reconnect...");
  ...
  let cv = document.getElementById("rd"),
  ...
  // 👇 đảm bảo canvas vẫn vẽ kể cả chưa có data
  window.onload = () => {
...
void processTarget(int16_t x, int16_t y, int16_t s, int id, JsonArray& root) {
  ...
  if (!t.potential_fall && !inSafeZone) {
    ...
    if (vY < -150 && (t.smooth_doppler > 80 || distanceMoved > 200)) 
    {
...
//   String out;
//   out.reserve(256); // 🔥 tránh realloc crash
...
void publishTracks() {
  ...
  for (int i = 1; i <= MAX_TRACKS; i++) {
    ...
    if (!isGhost) {
      processTarget((int16_t)tracks[i].x, (int16_t)tracks[i].y,
                    tracks[i].raw_speed, i, arr);
    } else {
      // Ghost: dùng lại action cuối cùng thay vì gán STILL
      JsonObject obj = arr.createNestedObject();
      obj["id"] = i;
      obj["x"] = (int)tracks[i].x;
      obj["y"] = (int)tracks[i].y;
      obj["spd"] = (int)abs(tracks[i].raw_speed);
      obj["dist"] = 0;
      obj["a"] = ts[i].action;   // ← giữ trạng thái cũ (FALL/FALLING/IMMOBILE)
      obj["ghost"] = true;
    }
  }
...
void handleRadar() {
  ...
  while (RADAR_SERIAL.available()) {
    ...
    if (idx == 30) {
      ...
      // check header + footer
      if (buf[0] == 0xAA && buf[1] == 0xFF && buf[28] == 0x55) {
...
void setup() {
  ...
  WiFi.setSleep(false); // 🔥 THÊM Ở ĐÂY (sau begin là đẹp nhất)
